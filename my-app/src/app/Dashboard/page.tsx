  ...
  const handleSendMessage = () => {
    if (newMessage.trim()) {
      setChatMessages(prevMessages => [
        ...prevMessages, 
        { id: prevMessages.length + 1, sender: 'User', message: newMessage },
        { 
          ...
          <div className="space-y-4">
            {contractTemplates.map((template) => (
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => setActiveContract(template.name)}
              ...
              >
                {/* {React.cloneElement(template.icon, {
                  className: `w-6 h-6 ${
                    activeContract === template.name ? 'text-blue-400' : 'text-gray-400'
                  }`
                })} */}
